...
for op in [
    ...
    elseif op in [:pushforward, :pullback]
        @eval function run_benchmark_aux(ba::AbstractADType, scen::$S1out)
            @compat (; f, x, seed, contexts) = deepcopy(scen)
            # benchmark
            ex = $prep_op(f, ba, x, seed, contexts...)
            bench0 = @be $prep_op(f, ba, x, seed, contexts...) samples = 1 evals = 1
            bench1 = @be ex $val_and_op(f, _, ba, x, seed, contexts...) evals = 1
            bench2 = @be ex $op(f, _, ba, x, seed, contexts...) evals = 1
            # count
            cc = CallCounter(f)
            ex = $prep_op(cc, ba, x, seed, contexts...)
            calls0 = reset_count!(cc)
            $val_and_op(cc, ex, ba, x, seed, contexts...)
            calls1 = reset_count!(cc)
            $op(cc, ex, ba, x, seed, contexts...)
            calls2 = reset_count!(cc)
            return (; bench0, bench1, bench2, calls0, calls1, calls2)
        end

        @eval function run_benchmark_aux(ba::AbstractADType, scen::$S1in)
            @compat (; f, x, seed, res1, contexts) = deepcopy(scen)
            # benchmark
            ex = $prep_op(f, ba, x, seed, contexts...)
            bench0 = @be $prep_op(f, ba, x, seed, contexts...) samples = 1 evals = 1
            bench1 = @be (res1, ex) $val_and_op!(f, _[1], _[2], ba, x, seed, contexts...) evals =
                1
            bench2 = @be (res1, ex) $op!(f, _[1], _[2], ba, x, seed, contexts...) evals = 1
            # count
            cc = CallCounter(f)
            ex = $prep_op(cc, ba, x, seed, contexts...)
            calls0 = reset_count!(cc)
            $val_and_op!(cc, res1, ex, ba, x, seed, contexts...)
            calls1 = reset_count!(cc)
            $op!(cc, res1, ex, ba, x, seed, contexts...)
            calls2 = reset_count!(cc)
            return (; bench0, bench1, bench2, calls0, calls1, calls2)
        end

        @eval function run_benchmark_aux(ba::AbstractADType, scen::$S2out)
            @compat (; f, x, y, seed, contexts) = deepcopy(scen)
            # benchmark
            ex = $prep_op(f, y, ba, x, seed, contexts...)
            bench0 = @be $prep_op(f, y, ba, x, seed, contexts...) samples = 1 evals = 1
            bench1 = @be (y, ex) $val_and_op(f, _[1], _[2], ba, x, seed, contexts...) evals =
                1
            bench2 = @be (y, ex) $op(f, _[1], _[2], ba, x, seed, contexts...) evals = 1
            # count
            cc = CallCounter(f)
            ex = $prep_op(cc, y, ba, x, seed, contexts...)
            calls0 = reset_count!(cc)
            $val_and_op(cc, y, ex, ba, x, seed, contexts...)
            calls1 = reset_count!(cc)
            $op(cc, y, ex, ba, x, seed, contexts...)
            calls2 = reset_count!(cc)
            return (; bench0, bench1, bench2, calls0, calls1, calls2)
        end

        @eval function run_benchmark_aux(ba::AbstractADType, scen::$S2in)
            @compat (; f, x, y, seed, res1, contexts) = deepcopy(scen)
            # benchmark
            ex = $prep_op(f, y, ba, x, seed, contexts...)
            bench0 = @be $prep_op(f, y, ba, x, seed, contexts...) samples = 1 evals = 1
            bench1 = @be (y, res1, ex) $val_and_op!(
                f, _[1], _[2], _[3], ba, x, seed, contexts...
            ) evals = 1
            bench2 = @be (y, res1, ex) $op!(f, _[1], _[2], _[3], ba, x, seed, contexts...) evals =
                1
            # count
            cc = CallCounter(f)
            ex = $prep_op(cc, y, ba, x, seed, contexts...)
            calls0 = reset_count!(cc)
            $val_and_op!(cc, y, res1, ex, ba, x, seed, contexts...)
            calls1 = reset_count!(cc)
            $op!(cc, y, res1, ex, ba, x, seed, contexts...)
            calls2 = reset_count!(cc)
            return (; bench0, bench1, bench2, calls0, calls1, calls2)
        end
    ...
    elseif op in [:hvp]
        ...
        @eval function run_benchmark_aux(ba::AbstractADType, scen::$S1in)
            @compat (; f, x, seed, res2, contexts) = deepcopy(scen)
            # benchmark
            ex = $prep_op(f, ba, x, seed, contexts...)
            bench0 = @be $prep_op(f, ba, x, seed, contexts...) samples = 1 evals = 1
            bench1 = @be +(1, 1) evals = 1  # TODO: fix
            bench2 = @be (res2, ex) $op!(f, _[1], _[2], ba, x, seed, contexts...) evals = 1
            # count
            cc = CallCounter(f)
            ex = $prep_op(cc, ba, x, seed, contexts...)
            calls0 = reset_count!(cc)
            calls1 = -1  # TODO: fix
            $op!(cc, res2, ex, ba, x, seed, contexts...)
            calls2 = reset_count!(cc)
            return (; bench0, bench1, bench2, calls0, calls1, calls2)
        end
    end
