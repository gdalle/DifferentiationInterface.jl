...
function DI.prepare_pullback(
    f, ::AutoZygote, x, ty::NTuple, contexts::Vararg{DI.Constant,C}
) where {C}
...
function DI.prepare_gradient(f, ::AutoZygote, x, contexts::Vararg{DI.Constant,C}) where {C}
...
function DI.prepare_jacobian(f, ::AutoZygote, x, contexts::Vararg{DI.Constant,C}) where {C}
...
function DI.value_and_jacobian!(
    f, jac, prep::DI.NoJacobianPrep, backend::AutoZygote, x, contexts::Vararg{DI.Constant,C}
) where {C}
...
function DI.prepare_hvp(
    f, backend::AutoZygote, x, tx::NTuple, contexts::Vararg{DI.Constant,C}
) where {C}
    return DI.prepare_hvp(f, DI.SecondOrder(AutoForwardDiff(), backend), x, tx, contexts...)
end

function DI.hvp(
    f, prep::DI.HVPPrep, backend::AutoZygote, x, tx::NTuple, contexts::Vararg{DI.Constant,C}
) where {C}
...
function DI.prepare_hessian(f, ::AutoZygote, x, contexts::Vararg{DI.Constant,C}) where {C}
    return DI.NoHessianPrep()
end

function DI.hessian(
    f, ::DI.NoHessianPrep, ::AutoZygote, x, contexts::Vararg{DI.Constant,C}
) where {C}
...
function DI.hessian!(
    f, hess, prep::DI.NoHessianPrep, backend::AutoZygote, x, contexts::Vararg{DI.Constant,C}
) where {C}
