...
function test_correctness_pushforward_allocating(ba::AbstractADType, scenario::Scenario)
    ...
    y_out1, dy_out1 = value_and_pushforward(ba, f, x, dx)
    dy_in2 = zero(dy_out1)
    y_out2, dy_out2 = value_and_pushforward!(dy_in2, ba, f, x, dx)

    dy_out3 = pushforward(ba, f, x, dx)
    dy_in4 = zero(dy_out3)
    dy_out4 = pushforward!(dy_in4, ba, f, x, dx)
...
function test_correctness_pushforward_mutating(ba::AbstractADType, scenario::Scenario)
    ...
    y_in = zero(y)
    dy_in = zero(dy_true)
    y_out, dy_out = value_and_pushforward!(y_in, dy_in, ba, f!, x, dx)
...
function test_correctness_pullback_allocating(ba::AbstractADType, scenario::Scenario)
    ...
    y_out1, dx_out1 = value_and_pullback(ba, f, x, dy)
    dx_in2 = zero(dx_out1)
    y_out2, dx_out2 = value_and_pullback!(dx_in2, ba, f, x, dy)

    dx_out3 = pullback(ba, f, x, dy)
    dx_in4 = zero(dx_out3)
    dx_out4 = pullback!(dx_in4, ba, f, x, dy)
...
function test_correctness_pullback_mutating(ba::AbstractADType, scenario::Scenario)
    ...
    y_in = zero(y)
    dx_in = zero(dx_true)
    y_out, dx_out = value_and_pullback!(y_in, dx_in, ba, f!, x, dy)
...
function test_correctness_derivative_allocating(ba::AbstractADType, scenario::Scenario)
    ...
    y_out1, der_out1 = value_and_derivative(ba, f, x)

    der_out2 = derivative(ba, f, x)
...
function test_correctness_multiderivative_allocating(ba::AbstractADType, scenario::Scenario)
    ...
    y_out1, multider_out1 = value_and_multiderivative(ba, f, x)
    multider_in2 = zero(multider_out1)
    y_out2, multider_out2 = value_and_multiderivative!(multider_in2, ba, f, x)

    multider_out3 = multiderivative(ba, f, x)
    multider_in4 = zero(multider_out3)
    multider_out4 = multiderivative!(multider_in4, ba, f, x)
...
function test_correctness_multiderivative_mutating(ba::AbstractADType, scenario::Scenario)
    ...
    y_out, multider_out = value_and_multiderivative!(y_in, multider_in, ba, f!, x)
...
function test_correctness_gradient_allocating(ba::AbstractADType, scenario::Scenario)
    ...
    y_out1, grad_out1 = value_and_gradient(ba, f, x)
    grad_in2 = zero(grad_out1)
    y_out2, grad_out2 = value_and_gradient!(grad_in2, ba, f, x)

    grad_out3 = gradient(ba, f, x)
    grad_in4 = zero(grad_out3)
    grad_out4 = gradient!(grad_in4, ba, f, x)
...
function test_correctness_jacobian_allocating(ba::AbstractADType, scenario::Scenario)
    ...
    y_out1, jac_out1 = value_and_jacobian(ba, f, x)
    jac_in2 = zero(jac_out1)
    y_out2, jac_out2 = value_and_jacobian!(jac_in2, ba, f, x)

    jac_out3 = jacobian(ba, f, x)
    jac_in4 = zero(jac_out3)
    jac_out4 = jacobian!(jac_in4, ba, f, x)
...
function test_correctness_jacobian_mutating(ba::AbstractADType, scenario::Scenario)
    ...
    y_out, jac_out = value_and_jacobian!(y_in, jac_in, ba, f!, x)
...
function test_correctness_second_derivative_allocating(
    ...
    y_out1, der_out1, derder_out1 = value_derivative_and_second_derivative(ba, f, x)

    derder_out2 = second_derivative(ba, f, x)
...
function test_correctness_hessian_allocating(ba::AbstractADType, scenario::Scenario)
    ...
    y_out1, grad_out1, hess_out1 = value_gradient_and_hessian(ba, f, x)
    grad_in2, hess_in2 = zero(grad_out1), zero(hess_out1)
    y_out2, grad_out2, hess_out2 = value_gradient_and_hessian!(grad_in2, hess_in2, ba, f, x)

    hess_out3 = hessian(ba, f, x)
    hess_in4 = zero(hess_out3)
    hess_out4 = hessian!(hess_in4, ba, f, x)

    hvp_out5 = hessian_vector_product(ba, f, x, dx)
    hvp_in6 = zero(hvp_out5)
    hvp_out6 = hessian_vector_product!(hvp_in6, ba, f, x, dx)
